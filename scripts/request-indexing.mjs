 ...
 *   4. Submits URLs to Google Indexing API, priority-sorted by SEO tier
 *   5. Saves the updated cache
 *
 * Priority ordering (within the 200 req/day quota):
 *   G/F tier → E tier → everything else
 *   Tier data comes from site-plan.json (generated by the SEO pipeline).
 *   If site-plan.json is missing, all URLs are treated as equal priority.
 *
 * Setup (one-time):
 ...
 *
 * Usage:
 *   node scripts/request-indexing.mjs              # new URLs only, priority-sorted
 *   node scripts/request-indexing.mjs --all        # re-submit ALL URLs, priority-sorted
 *   node scripts/request-indexing.mjs --priority   # only submit G/F/E tier pages
...
const SITE_PLAN_FILE = path.join(__dirname, '..', 'site-plan.json');
...
// ── SEO tier priority (from site-plan.json) ───────────────────────────────────
/**
 * Returns a sort key so G/F pages come first, then E, then everything else.
 * Lower number = submitted sooner within the 200/day quota.
 */
function loadPrioritySet() {
  try {
    const plan = JSON.parse(fs.readFileSync(SITE_PLAN_FILE, 'utf-8'));
    const urls  = plan?.sitemaps?.priority?.urls ?? [];
    // Map absolute URL → tier ('G', 'F', 'E', ...)
    const tierMap = new Map();
    for (const entry of urls) {
      tierMap.set(SITE_URL + entry.url, entry.tier ?? 'E');
    }
    return tierMap;
  } catch {
    return new Map();
...
function tierSortKey(tier) {
  const order = { G: 0, F: 1, E: 2 };
  return order[tier] ?? 3;
}

function sortByTier(urls, tierMap) {
  return [...urls].sort((a, b) => {
    const ta = tierSortKey(tierMap.get(a));
    const tb = tierSortKey(tierMap.get(b));
    return ta - tb;
  });
}
...
async function main() {
  ...
  const modeLabel = DRY ? 'DRY RUN' : PING ? 'PING ONLY' : ALL ? 'ALL URLs' : PRIORITY_ONLY ? 'PRIORITY ONLY (G/F/E tier)' : 'NEW URLs only (priority-sorted)';
  ...
  // Load SEO tier data for priority ordering
  const tierMap = loadPrioritySet();
  if (tierMap.size > 0) {
    console.log(`[INFO] Loaded ${tierMap.size} priority URLs from site-plan.json`);
  }
  ...
  // --priority: only submit pages that appear in the site-plan priority list
  if (PRIORITY_ONLY) {
    const before = toSubmit.length;
    toSubmit = toSubmit.filter((url) => tierMap.has(url));
    console.log(`[INFO] --priority: filtered to ${toSubmit.length} of ${before} URLs (G/F/E tier only)`);
  }

  // Sort so highest-tier pages are submitted first within the daily quota
  toSubmit = sortByTier(toSubmit, tierMap);
